...
public class HomeController {
    ...
    public String home(Model model) {
        // 컨트롤러 메서드에 필요한 객체를 매개변수로 작성해주면
        // Spring Container 가 Arguments Resolver 를 이용하여
        // 1) 해당 Bean 이 존재하면 주입
        // 2) 없으면 새로운 Bean 을 생성해서 주입
        // => new 를 쓸 일이 없음
        log.info("================> HomeController /");
        model.addAttribute("title", "Spring"); // View로 전달할 데이터 Model에 추가
